...
return baseclass.extend({
    ...
    renderHtml: function(data, type) {
        ...
        for (var idx in data) {
            ...
            if (['dnsv4'].includes(idx) && Array.isArray(val)) {
                container_item.appendChild(this.renderArrayAsTable(titleStr, val));
            } else {
    ...
    startPolling: function() {
        ...
        var updateInternet = function() {
            ...
            Promise.all([
                network.flushCache().then(function() { return network.getWANNetworks(); }),
                fetch('/cgi-bin/internet_check').then(function(r) { return r.text(); }).catch(function() { return 'false'; }),
                fetch('/cgi-bin/real_ip').then(function(r) { return r.text(); }).catch(function() { return '-'; })
            ]).then(function(results) {
                var fakeData = [results[0], null, null, null, null, null, results[1], results[2]];
                var newBox = self.renderInternetBox(fakeData);
                var oldBox = document.querySelector('.internet-status-self');
                if (oldBox && newBox) {
                    // Preserve VPN widget if attached
                    var vpnWidget = oldBox.querySelector('.vpn-quota-widget');
                    oldBox.parentNode.replaceChild(newBox, oldBox);
                    if (vpnWidget) newBox.appendChild(vpnWidget);
                }
            }).catch(function() {})
            .finally(function() {
